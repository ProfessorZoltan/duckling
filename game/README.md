...
2. Press **F5**. You start inside the egg: mash **Space** to peck your way out. Say hello to Marra and each sibling with **E** (the prompt names the nearest duck), then follow Marra and the siblings round the pond (she waits for you at each stop), eat five water bugs when she tells you to, then hide under the reeds three times when the hawk's shadow comes. That's Growth Spurt 1. The family leaves through the culvert and the pond goes gray; push through the reeds to the east, talk to Nib with **E**, and bring back eight seeds. **G** forces a spurt at any time. Press **F2** to jump to the cliff flight course, **F1** to come back.
...
| `world/pond_greybox.gd` | The act director: Prologue → Hello (greet all five before Marra sets off) → Keep Up → First Supper → The Shadow → Growth Spurt 1 → The Far Shore and the Culvert (siblings through, Marra flies, Heart 10) → Nib and her pantry. Sets the objective line and Marra's and Pip's lines at each beat. |

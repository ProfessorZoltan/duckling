...
func _physics_process(_delta: float) -> void:
	...
	match frame:
		...
		1500:
			print("heart: frame 1500 heart=%.0f displayed=%.0f grass=%s" % [Globals.heart, region.displayed_heart, grass.albedo_color])
			...
			var sib: Node3D = pond.siblings[0]
		...
		1700:
			...
			for seed in get_tree().get_nodes_in_group("pickup_seed"):
				...
				await get_tree().physics_frame
		1760:
			print("heart: frame 1760 seeds=%d heart=%.0f" % [pond.seeds_collected, Globals.heart])
		...
		2100:
			print("heart: frame 2100 displayed=%.0f grass=%s original=%s" % [region.displayed_heart, grass.albedo_color, grass_original])
